...
class Seq2SeqAgent(BaseAgent):
    ...
    def __init__(self, env, results_path, tok, episode_len=20,encoder_type='EncoderLSTM'):
        ...
        if self.encoder_type in ['EncoderLSTM','DicEncoder','CEncoder']:
            ...
            self.d_decoder = model.BAttnDecoderLSTM(args.aemb, args.d_hidden_size, args.dropout,
        ...
        self.d_decoder_optimizer = args.optimizer(self.d_decoder.parameters(), lr=args.lr)
        if args.use_lr_scheduler:
            self.lr_scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(self.decoder_optimizer,
                                                                      mode='min',
                                                                      factor=0.8,
                                                                      patience=100)
            self.d_lr_scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(self.decoder_optimizer,
                                                                           mode='min',
                                                                           factor=0.8,
                                                                           patience=100)
        else:
        ...
        self.models = (self.encoder, self.decoder, self.d_decoder, self.critic)
        self.optimizers = (self.encoder_optimizer, self.decoder_optimizer, self.d_decoder_optimizer, self.critic_optimizer)
    ...
    def vl_rollout(self, train_ml=None, train_rl=True, reset=True, speaker=None):
        ...
        for t in range(self.episode_len):
            ...
            if t == 0:
                ih_t, ic_t, ilogit, ih1, iback_logit = self.decoder(input_a_t, f_t,  candidate_feat,
                                                   en_ht, en_ht, en_ct,
                                                   ctx, ctx_mask,
                                                   already_dropfeat=(speaker is not None))

                dh_t, dc_t, dlogit, dh1, dback_logit = self.d_decoder(input_a_t, d_t, candidate_dfeat,
                                                               en_ht, en_ht, en_ct,
                                                               ctx, ctx_mask,
                                                               already_dropfeat=(speaker is not None))
            ...
            h1 = ih1 + dh1
            if args.pred_back:
                back_logit = iback_logit + dback_logit
        ...
        if train_rl:
            ...
            if speaker is not None:
                candidate_feat[..., :-args.angle_feat_size] *= noise
                f_t[..., :-args.angle_feat_size] *= noise
            i_last_h_, _, _, _, _ = self.decoder(input_a_t, f_t, candidate_feat,
                                            h_t, h1, c_t,
                                            ctx, ctx_mask,
                                            speaker is not None)
            d_last_h_, _, _, _, _ = self.d_decoder(input_a_t, d_t, candidate_dfeat,
                                                                  h_t, h1, c_t,
                                                                  ctx, ctx_mask,
                                                                  already_dropfeat=(speaker is not None))
            last_h_ = i_last_h_ + d_last_h_
            ...
            for t in range(length-1, -1, -1):
                ...
                # r_: The higher, the better. -ln(p(action)) * (discount_reward - value)
                rl_loss += (-policy_log_probs[t] * a_ * mask_).sum() # r dp -> r p 1/p dp -> \sum r p d log p -> E[r d logp] = 1/N sum(r_i + d log p_i)
                rl_loss += (((r_ - v_) ** 2) * mask_).sum() * 0.5     # 1/2 L2 loss
    ...
    def optim_step(self, **kwargs):
        ...
        if self.d_lr_scheduler is None:
            self.d_decoder_optimizer.step()
        else:
            self.d_lr_scheduler.step(kwargs['metrics'])
    ...
    def train(self, n_iters, feedback='teacher', **kwargs):
        ...
        for iter in range(1, n_iters + 1):
            ...
            self.d_decoder_optimizer.zero_grad()
    ...
    def save(self, epoch, path):
        ...
        all_tuple = [("encoder", self.encoder, self.encoder_optimizer),
                     ...
                     ("d_decoder", self.d_decoder, self.d_decoder_optimizer),
