...
class Seq2SeqAgent(BaseAgent):
    ...
    def __init__(self, env, results_path, tok, episode_len=20,encoder_type='EncoderLSTM'):
        ...
        if self.encoder_type in ['EncoderLSTM','DicEncoder','CEncoder']:
            ...
            self.decoder = model.KVMemAttnDecoderLSTM(args.aemb, args.d_hidden_size, args.dropout, feature_size=self.feature_size + args.angle_feat_size, pred_back=args.pred_back).cuda()

        self.critic = model.Critic().cuda()
        if args.adaIn_type == "channel":
            self.adaIn = DGAdaChannel(args.feature_size)
            self.adaIn.cuda()
        elif args.adaIn_type == "meanchannel":
        ...
        def lr_lambda(iter_count): # log_every 100
            ...
            if iter_count < 5*decay_intervals:
                alpha = 1.0
            elif iter_count < 10*decay_intervals:
                alpha = 0.5
            elif iter_count < 20*decay_intervals:
                alpha = 0.2
            elif iter_count < 40*decay_intervals:
                alpha = 0.1
            elif iter_count < 80 * decay_intervals:
                alpha = 0.05
            else:
                alpha = 0.02
            return alpha
    ...
    def vl_rollout(self, train_ml=None, train_rl=True, reset=True, speaker=None):
        ...
        for t in range(self.episode_len):
            ...
            elif args.adaIn_type in ["channel", "meanchannel"]:
                df_t[:, :, :-args.angle_feat_size] = self.adaIn(
                    f_t[:, :, :-args.angle_feat_size].clone(), d_t[:, :, :-args.angle_feat_size].clone())
                candidate_feat[:, :, :-args.angle_feat_size] = self.adaIn(
            ...
            if args.include_vision:
                if args.use_dropout_vision:
                    if self.encoder.__class__.__name__ == 'DicEncoder':
                        ...
                        ctx, en_ht, en_ct, _, _ = self.encoder(seq, mask=seq_mask, lengths=torch.tensor(seq_lengths),f_t_all=f_t.clone())
                    else:
            ...
            if t == 0:
                h_t, c_t, logit, h1, back_logit = self.decoder(input_a_t, df_t, candidate_feat,
                                                   en_ht, en_ht, en_ct,
                                                   ctx, ctx_mask,
                                                   already_dropfeat=(speaker is not None))
            else:
            ...
            ml_loss += forth_loss
            self.logs['forth_loss'].append(forth_loss.detach().cpu().item())

            if args.pred_back:
                back_logit.masked_fill_(candidate_mask, -float('inf'))
                back_target = self._back_teacher_action(perm_obs, ended)
                back_loss = self.criterion(back_logit, back_target)
                self.logs['back_loss'].append(back_loss.detach().cpu().item())
                ml_loss += back_loss
        ...
        if train_rl:
            ...
            #     print(c)
            # print('='*30)
            # for c in discount_reward:
            #     print(c)

            # print("args.normalize_loss", args.normalize_loss)
            # Normalize the loss function
            if args.normalize_loss == 'total':
                rl_loss /= total
            elif args.normalize_loss == 'batch':
