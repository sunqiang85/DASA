...
class DicModel(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.vision_encoder = VisionEncoder(self.config.img_feature_dim, self.config)
        if args.d_v_layers > 0:
            self.vlayer =  nn.ModuleList(
                [BertLayer(config) for _ in range(args.d_v_layers)]
            )
        # from types import MethodType
        # for attr in dir(config):
        #     if not attr.startswith('__') and not isinstance(getattr(config, attr), MethodType):
        #         print('config: { %-17s }->' % attr, getattr(config, attr))


        # print("self.addlayer[0].lang_inter.intermediate_act_fn", type(self.addlayer[0].lang_inter.intermediate_act_fn), self.addlayer[0].lang_inter.intermediate_act_fn)
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, position_ids=None, head_mask=None, img_feats=None):
        ...
        # print("self.update_lang_bert", self.update_lang_bert)
        # print("self.update_add_layer", self.update_add_layer)
        ...
        if img_feats is not None:
            ...
            if args.d_v_layers > 0:
                for layer_module in self.vlayer:
                    temp_output= layer_module(visn_output, img_mask)
                    visn_output = temp_output[0]
        ...
        #outputs = (sequence_output, pooled_output,) + encoder_outputs[1:]  # add hidden_states and attentions if they are here
        outputs = (sequence_output, pooled_output, visn_output)  # add hidden_states and attentions if they are here
