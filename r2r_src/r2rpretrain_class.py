...
class DicPMActionPreTrain(BertPreTrainedModel):
    def __init__(self,config, max_seq_len=80):
        super(DicPMActionPreTrain, self).__init__(config)
        ...
        self.critic = nn.Sequential(
            nn.Linear(max_seq_len  + self.config.hidden_size, 1),
            nn.Sigmoid()
        )
        self.pm_loss_func = nn.MSELoss()
    ...
    def forward(self, seq,labels, f_t_all = None, isnext=None, lang_mask=None, progresses=None, tasks=['lmask'], loss_weights={}):
        ...
        if 'lmask' in tasks:
            prediction_scores = self.mlmhead(lang_part)
            mask_loss = self.criterion(prediction_scores.view(-1,self.config.vocab_size), labels.view(-1))
            losses['lmask_loss'] = mask_loss
            scores['lmask_scores'] = prediction_scores
            lm_weight = loss_weights.get('lmask', 1.0)
            loss = loss + mask_loss*lm_weight


        # action task
        if 'action' in tasks:
            action_scores = self.next_action(cls_part)
            action_loss = self.criterion(action_scores, isnext)
            losses['action_loss'] = action_loss
            scores['action_scores'] = action_scores
            action_weight = loss_weights.get("action", 1.0)
            loss = loss + action_loss*action_weight


        # progress monitor task
        if 'pm' in tasks:
            # print("progress.shape", progresses.shape)
            # print("cls_part", cls_part.shape)
            # print("lang_part", lang_part.shape)
            att_cls, att_map = attention(lang_part, lang_part, cls_part.unsqueeze(1))
            att_cls = att_cls.squeeze(1)
            att_map = att_map.squeeze(1)

            rev_mask = ~lang_mask
            reversed_att = torch.zeros(att_map.size()).to(att_map.device)
            reverse_idx = torch.arange(att_map.shape[-1] - 1, -1, -1)
            reversed_att[rev_mask] = att_map[:, reverse_idx][rev_mask[:, reverse_idx]]

            pm_input = torch.cat([cls_part, reversed_att], dim =-1)
            pm_scores = self.critic(pm_input).squeeze(-1)
            pm_loss = self.pm_loss_func(pm_scores, progresses)
            losses['pm_loss']  = pm_loss
            scores['pm_scores'] = pm_scores
            pm_weight = loss_weights.get("pm", 1.0)
            loss = loss + pm_loss*pm_weight
