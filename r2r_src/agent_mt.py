...
from utils import padding_idx, add_idx, Tokenizer, ViewHelper, get_distance_weight
import utils
import model
...
class Seq2SeqAgent(BaseAgent):
    ...
    def _candidate_variable(self, obs):
        ...
        candidate_idx = np.ones((len(obs), max(candidate_leng)), dtype=np.int) * -1
        # Note: The candidate_feat at len(ob['candidate']) is the feature for the END
        # which is zero in my implementation
        for i, ob in enumerate(obs):
            for j, c in enumerate(ob['candidate']):
                candidate_feat[i, j, :] = c['feature']                         # Image feat
                candidate_idx[i,j] = c['pointId']  # idx in the current viewpoint
        return torch.from_numpy(candidate_feat).cuda(), candidate_leng, torch.from_numpy(candidate_idx).cuda()
    ...
    def get_input_feat(self, obs):
        ...
        candidate_feat, candidate_leng, candidate_idx = self._candidate_variable(obs)

        return input_a_t, f_t, candidate_feat, candidate_leng, candidate_idx
    ...
    def _teacher_action(self, obs, ended):
        ...
        target_viewids = np.ones(len(obs), dtype=np.int64) * args.ignoreid
        for i, ob in enumerate(obs):
            if ended[i]:                                            # Just ignore this index
                a[i] = args.ignoreid
                target_viewids[i] = args.ignoreid

            else:
                for k, candidate in enumerate(ob['candidate']):
                    if candidate['viewpointId'] == ob['teacher']:   # Next view point
                        a[i] = k
                        target_viewids[i] = candidate['pointId']
                        break
                else:   # Stop here
                    assert ob['teacher'] == ob['viewpoint']         # The teacher action should be "STAY HERE"
                    a[i] = len(ob['candidate'])
                    target_viewids[i] = args.ignoreid
        return torch.from_numpy(a).cuda(), torch.from_numpy(target_viewids).cuda()
    ...
    def rollout(self, train_ml=None, train_rl=True, reset=True, speaker=None):
        ...
        for t in range(self.episode_len):
            ...
            input_a_t, f_t, candidate_feat, candidate_leng, candidate_idx = self.get_input_feat(perm_obs)
            ...
            # Supervised training
            target, target_viewids = self._teacher_action(perm_obs, ended)
    ...
    def vl_rollout(self, train_ml=None, train_rl=True, reset=True, speaker=None):
        ...
        for t in range(self.episode_len):
            ...
            # Supervised training
            target, target_viewids = self._teacher_action(perm_obs, ended)
            # print("ended", type(ended), ended)
            # print("target", target.shape, target) # b
            # print("target_viewids", type(target_viewids), target_viewids.shape, target_viewids)  # b
            # print("logit", logit.shape, logit[3]) # b, max_can
            # print("candiate_idx", candidate_idx.shape, candidate_idx[3])
            # print("args.ignoreid", args.ignoreid) -100
            target_soft_dist = get_distance_weight(target_viewids.cpu(), candidate_idx.cpu())

            has_action = ~(ended | target_viewids.cpu().numpy()==args.ignoreid)
            input_probs = torch.softmax(logit, dim=-1)
            input_dist = input_probs[has_action][candidate_idx[has_action]!=-1]
            target_dist = target_soft_dist[has_action][candidate_idx[has_action] != -1]
            kl_loss += F.kl_div(input_dist.log().float(), target_dist.float(), reduction='mean')
        ...
        self.logs['kl_loss'].append(kl_loss.detach().cpu().item())
        ...
        if train_ml is not None:
            self.loss += ml_loss * train_ml / batch_size
            self.loss += kl_loss * train_ml / batch_size
    ...
    def _dijkstra(self):
        ...
        for _ in range(300):
            ...
            input_a_t, f_t, candidate_feat, candidate_leng, candidate_idx = self.get_input_feat(obs)
