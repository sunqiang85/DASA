...
def load_nav_graphs(scans):
    ...
    graphs = {}
    with tqdm(total=(len(scans)), position=0, leave=True, ascii=True) as pbar:
        for scan in scans:
...
def load_pretrain_datasets(splits):
    """
    ...
    for split in splits:
        with open('tasks/R2R/data/R2R_%s.json' % split) as f:
            ...
            for item in src_data:
                src_dict[item['path_id']] = {
                    'path': item['path'],
                    'instructions': item['instructions']
                }
        with open('tasks/R2R/data/target_%s.json' % split) as f:
            new_data = json.load(f)
            for d in new_data:
                d['instructions'] = src_dict[d['path_id']]['instructions']
                d['path'] = src_dict[d['path_id']]['path']
            data += new_data
...
def load_datasets(splits):
    ...
    for split in splits:
        ...
        if "/" not in split:
            if args.mini:
                filename = 'tasks/R2R/mini_data/R2R_%s.json' % split
            else:
...
def read_img_features(feature_store):
    ...
    if args.mini:
        features = {}
        idx_array = np.load(args.IMAGENET_FEATURES_INDEX)
        val_array = np.load(args.IMAGENET_FEATURES_VALUE)
        for k, v in zip(idx_array, val_array):
            features[k] = v
...
def attention(value, key, query, mask=None, output_prob=True):
    """
    :param value: b, l_k, d
    :param key: b, l_k, d
    :param query: b, l_q, d or b, d
    :param mask:
    :return: b, l_q, d or b, d
    """
    input_query_size = len(query.shape)
    if input_query_size == 2:
    ...
    att_result = torch.matmul(att_map, value)

    if input_query_size == 2:
        att_result = att_result.squeeze(1)
        att_map = att_map.squeeze(1)

    return att_result, att_map
...
class ViewHelper():
    def __init__(self):
        self.abs_angles = self.get_abs_angles()
    ...
    def get_abs_angles(self):
        ...
        for ix in range(36):
            ...
            abs_angles.append([state.elevation, state.heading])
        return np.array(abs_angles)

    def get_target_rel_weight(self, target):
        base_elevation = ((target // 12) - 1) * np.pi / 6
        base_heading = (target % 12) * np.pi / 6
        base_point = np.array([base_elevation, base_heading])
        round_point = np.array([0, np.pi * 2])
        diff = self.abs_angles - base_point
        rel_diff = np.abs(np.stack([diff, round_point - diff])).min(axis=0)
        rel_diff = np.sqrt((rel_diff * rel_diff).sum(axis=-1))
        rel_weight = -rel_diff * 4
        return rel_weight

VHelper = ViewHelper()
def get_distance_weight(target_idx, candidate_idx):
    candidate_distance_weight = []
    for target_id, candidate_ids in zip(target_idx, candidate_idx):
        weights = VHelper.get_target_rel_weight(target_id)
        candidate_distance_weight.append(weights[candidate_ids])
    r = torch.from_numpy(np.array(candidate_distance_weight))
    r.masked_fill_(candidate_idx==-1, -100000)
    r = torch.softmax(r, -1)
    return r.cuda()
