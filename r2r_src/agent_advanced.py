...
class Seq2SeqAgent(BaseAgent):
    ...
    def __init__(self, env, results_path, tok, episode_len=20,encoder_type='EncoderLSTM'):
        ...
        if self.encoder_type in ['EncoderLSTM','DicEncoder','CEncoder']:
            ...
            if args.decoder_type == 'advanced':
                self.decoder = model.AdvancedDecoderLSTM(args.aemb, args.d_hidden_size, args.dropout, feature_size=self.feature_size + args.angle_feat_size, pred_back=args.pred_back).cuda()
            else:
                self.decoder = model.BAttnDecoderLSTM(args.aemb, args.d_hidden_size, args.dropout,
                                                      feature_size=self.feature_size + args.angle_feat_size,
                                                      pred_back=args.pred_back).cuda()
        ...
        if args.use_lr_scheduler:
            self.lr_scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(self.decoder_optimizer,
                                                                      mode='min',
                                                                      factor=0.8,
                                                                      patience=100)
        else:
    ...
    def _sort_batch(self, obs):
        ...
        gt_progress = np.array([ob['progress'] for ob in obs])
        ...
        gt_progress = torch.from_numpy(gt_progress)[perm_idx]
        return Variable(sorted_tensor, requires_grad=False).long().cuda(), \
               mask.bool().cuda(),  \
               list(seq_lengths), list(perm_idx), gt_progress.cuda()
        # return Variable(sorted_tensor, requires_grad=False).long().cuda(), \
        #        mask.byte().cuda(), \
    ...
    def vl_rollout(self, train_ml=None, train_rl=True, reset=True, speaker=None):
        ...
        seq, seq_mask, seq_lengths, perm_idx, gt_progress = self._sort_batch(obs)
        ...
        for t in range(self.episode_len):
            ...
            if t == 0:
                h_t, c_t, logit, h1, back_logit, pred_progress = self.decoder(input_a_t, df_t, candidate_feat,
                                                   en_ht, en_ht, en_ct,
                                                   ctx, ctx_mask,
            ...
            ml_loss += forth_loss
            pm_loss = ((gt_progress-pred_progress)*(gt_progress-pred_progress)).mean()
            ml_loss += 10 * pm_loss
            self.logs['pm_loss'].append(pm_loss.detach().cpu().item())
