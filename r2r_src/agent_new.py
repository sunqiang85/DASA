...
class Seq2SeqAgent(BaseAgent):
    ...
    def __init__(self, env, results_path, tok, episode_len=20,encoder_type='EncoderLSTM'):
        ...
        if self.encoder_type in ['EncoderLSTM','DicEncoder','CEncoder']:
            ...
            self.decoder = model.NewAttnDecoderLSTM(args.aemb, args.d_hidden_size, args.dropout, feature_size=self.feature_size + args.angle_feat_size, pred_back=args.pred_back).cuda()
        ...
        if self.encoder.__class__.__name__ == 'EncoderLSTM':
            ...
            if args.schedule_ratio == -1:
                if feedback == 'teacher':
                    ...
                    self.feedback = 'sample'
                    self.vl_rollout(train_ml=args.ml_weight, train_rl=True, **kwargs) # modify here
                else:
                    assert False
            else:
