...
class Seq2SeqAgent(BaseAgent):
    ...
    def __init__(self, env, results_path, tok, episode_len=20,encoder_type='EncoderLSTM'):
        ...
        # print("tok.vocab_size()", tok.vocab_size()) # vocab.txt + 1

        # Models
        enc_hidden_size = args.rnn_dim//2 if args.bidir else args.rnn_dim
        if self.encoder_type == 'McattEncoder':
            self.encoder = model.McattEncoder(args, tok.vocab_size()).cuda()


        if self.encoder_type == 'McattEncoder':
            # self.decoder = model.McattDecoder(args).cuda()
            self.decoder =  model.McattDecoder(args.aemb, args.HIDDEN_SIZE, args.dropout, feature_size=self.feature_size + args.angle_feat_size).cuda()
    ...
    def vl_rollout(self, train_ml=None, train_rl=True, reset=True, speaker=None, is_train = False):
        """
        :param train_ml:    The weight to train with maximum likelihood
        :param train_rl:    whether use RL in training
        :param reset:       Reset the environment
        ...
        if speaker is not None:         # Trigger the self_train mode!
            ...
            speaker.env = self.env
            insts = speaker.infer_batch(featdropmask=noise)     # Use the same drop mask in speaker
            inst_lengths = np.argmax(insts==self.tok.word_to_index['<PAD>'],axis=1)
        ...
        for t in range(self.episode_len):
            print("   t:", t)
            if args.backward_inrollout and is_train:
            ...
            # print("args.include_vision",args.include_vision) # True
            # print("args.use_dropout_vision",args.use_dropout_vision) # True

            if args.include_vision:
                if args.use_dropout_vision:
                    ctx, attended_txt, v_feat, attended_v = self.encoder(seq.clone(), seq_mask=seq_mask.clone(), seq_lengths=torch.tensor(seq_lengths), f_t_all=f_t.clone()) # B, 1, H_DIM(768)
            ...
            # if t == 0:
            #     logit = self.decoder(attended_v, input_a_t, f_t, candidate_feat,
            #                                        already_dropfeat=(speaker is not None))
            # else:
            #     logit = self.decoder(attended_v, input_a_t, f_t, candidate_feat,
            #                                        already_dropfeat=(speaker is not None))
            ...
            if t == 0:
                # print("attend_txt", attended_txt.shape)
                # print("attend_v", attended_v.shape)

                h_t, c_t, logit, h1 = self.decoder(input_a_t, f_t, candidate_feat,
                                                   attended_txt, attended_txt, attended_v.squeeze(1),
                                                   ctx, ctx_mask.clone(),
                                                   already_dropfeat=(speaker is not None))
            else:
                h_t, c_t, logit, h1 = self.decoder(input_a_t, f_t, candidate_feat,
        ...
        if train_ml is not None:
            if args.backward_inrollout:
                self.loss += ml_loss.detach().cpu() * train_ml / batch_size
    ...
    def train(self, n_iters, feedback='teacher', **kwargs):
        ...
        self.losses = []
        print("args.schedule_ratio", args.schedule_ratio)
        print("feedback", feedback) # sample

        for iter in range(1, n_iters + 1):
            print("train inter {}".format(iter))
            if not args.backward_inrollout:
                self.encoder_optimizer.zero_grad()
                self.decoder_optimizer.zero_grad()
                self.critic_optimizer.zero_grad()
            ...
            if self.encoder.__class__.__name__ == 'EncoderLSTM':
                ...
                if args.schedule_ratio == -1:
                    if feedback == 'teacher':
                        self.feedback = 'teacher'
                        self.vl_rollout(train_ml=args.teacher_weight, train_rl=False, is_train=True, **kwargs)
                    elif feedback == 'sample':
                        if args.ml_weight != 0:
                            self.feedback = 'teacher'
                            self.vl_rollout(train_ml=args.ml_weight, train_rl=False, is_train=True, **kwargs)
                        self.feedback = 'sample'
                        self.vl_rollout(train_ml=None, train_rl=True, **kwargs)
                    else:
                        assert False
                    ...
                    if feedback == 'teacher':
                        ...
                        self.feedback = 'sample'
                        self.vl_rollout(train_ml=None, train_rl=True, is_train=True, **kwargs)
                    else:
                        assert False
