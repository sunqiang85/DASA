...
class DicEncoder(nn.Module):
    ...
    def __init__(self, vision_size, hidden_size, dec_hidden_size, dropout_ratio, bidirectional, update, bert_n_layers, reverse_input, top_lstm, vl_layers, la_layers, bert_type="small",update_add_layer=True):
        ...
        if args.ctx_v:
            self.ctx_v_to_v = nn.Linear(self.transformer_hidden_size, 2048 + args.angle_feat_size)
    ...
    def forward(self, inputs, mask, lengths,f_t_all=None):
        ...
        # print("f_t_all", f_t_all.shape)

        batch_size = inputs.size(0)
        seq_max_len = mask.size(1)
        att_mask = ~mask
        # print("self.reverse_input", self.reverse_input)
        # print("self.top_lstm", self.top_lstm)
        # print("att_mask", type(att_mask), att_mask.dtype, att_mask[5])
        # print("mask", type(mask), mask.dtype, mask[5])
        # print("seq", inputs[5])
        # print("length", lengths[5])
        # print("self.num_directions", self.num_directions)
        # print("self.hidden_size * self.num_directions", self.hidden_size * self.num_directions)
        ...
        #                                  attention_mask=att_mask)
        #text_embeds = torch.cat(all_encoder_layers[-self.bert_n_layers:],-1)
        # only reserse the unmasked token
        outputs = self.bert(inputs[:, :seq_max_len], token_type_ids=None,
                                          attention_mask=att_mask, img_feats=f_t_all)
        # embeds: b, max_seq_len, hid_dim(768)
        # pooled_output: b, hid_dim(768 the cls token)
        # vision_outputs:
        embeds, pooled_output, vision_outputs = outputs[0], outputs[1], outputs[2]
        ...
        ctx = self.drop(ctx)
        if args.ctx_v:
            vision_outputs = self.ctx_v_to_v(vision_outputs)
        return ctx,decoder_init,c_t, mask, vision_outputs  # (batch, seq_len, hidden_size*num_directions)
