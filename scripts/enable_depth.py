        ...
        for skybox_ix in range(6):
            ...
            for camera in range(3):
                for angle in range(6):
                    ...
                    mask = cv2.warpPerspective(np.ones_like(flip), H, (SKYBOX_HEIGHT, SKYBOX_WIDTH),
            ...
            depth_small = cv2.resize(cv2.flip(base_depth, 1), (DOWNSIZED_WIDTH, DOWNSIZED_HEIGHT),
                                     interpolation=cv2.INTER_NEAREST)  # flip around y-axis, downsize
            if fill_holes:
